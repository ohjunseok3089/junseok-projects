...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # find the minimum maze distance to the closest ghost
        for livingGhost in livingGhostPositionDistributions:
            pos2 = livingGhost.argMax()
            distance = self.distancer.getDistance(pos1, pos2)
            if (minimumDist > distance):
                minimumDist = distance
                ghostPosition = pos2
                
        minimumDist = 99999999999909999999
        optimalAction = None
        # find successor and find minimum distance from ghost to successor'
        for action in legal:
